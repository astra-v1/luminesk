...
def _check_repository(core: CoreProvider) -> DiagnosticResult:
	try:
		...
		with httpx.Client(timeout=REPOSITORY_CHECK_TIMEOUT, follow_redirects=True) as client:
...
def _check_source(client: httpx.Client, core_name: str, check_url: str) -> DiagnosticResult:
	try:
		...
		return DiagnosticResult(
			name=t("diagnostic.source_name", core_name=core_name),
			status=False,
			message=t("diagnostic.source_http_status", status_code=response.status_code),
		)
	except Exception as exc:
		return DiagnosticResult(
			name=t("diagnostic.source_name", core_name=core_name),
